...
## Contexte produit
Le **hub / site vitrine** de la suite Aevia (la porte d'entrée commerciale). Périmètre plus léger que les apps, mais c'est **la première impression** — donc la barre visuelle, la perf, le SEO et l'exactitude du contenu doivent être irréprochables (c'est ce qui vend le high-ticket avant même la démo).

## Stack & points d'entrée
- Next.js (Vercel). `app/` (pages + i18n next-intl), `middleware.ts`, `app/api/contact/route.ts` (formulaire), `components/`, `messages/` (traductions).

## À VÉRIFIER / POLIR en priorité
- **Exactitude du contenu.** Prix, offres, promesses, mentions légales (SIREN 852 546 225), coordonnées — **doivent être justes et à jour**. Un prix périmé sur la vitrine tue la crédibilité. Vérifie contre la réalité produit.
- **SEO / GEO.** `llms.txt` / `llms-full.txt`, JSON-LD, hreflang / `x-default`, sitemap, métadonnées, Open Graph. Cohérence multilingue.
- **i18n.** Vérifie que toutes les locales déclarées ont leurs traductions (attention aux locales déclarées sans fichier → fallback). Zéro texte manquant / clé brute affichée.
- **Formulaire contact** (déjà audité, bien gardé — confirme) : `escapeHtml`, garde-fous longueur, regex email, rate-limit, Resend (pas de SMTP brut). Anti-spam suffisant ?
- **Perf & a11y.** Core Web Vitals, images optimisées (`next/image`, `max-width:100%`), Lighthouse, contraste, navigation clavier, alt.
- **Liens.** Aucun lien mort (interne/externe), CTA qui pointent bien vers les bons produits.

## Parcours client à tester (comme un vrai visiteur)
Arrivée landing → parcours des produits → tarifs → formulaire de contact → liens vers les apps. Desktop + mobile. Chaque page charge vite, zéro erreur console, contenu juste, rendu propre dans les 2 thèmes (clair/sombre si applicable).

## Note
Pas de multi-tenant ici (site public). L'isolation ne s'applique pas ; concentre-toi sur qualité, perf, SEO, exactitude, a11y, i18n.
